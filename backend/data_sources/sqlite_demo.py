"""Shared SQLite access for surveillance demo data."""
...
ALL_TABLES = (*CORE_TABLES, "market_ticks", "comms_messages")
...
def _rows(sql: str, params: dict[str, Any] | None = None, *, path: str | None = None) -> list[dict[str, Any]]:
...
def _context_from_filters(
    *,
    ...
    rows = _rows(f"SELECT * FROM hs_alerts WHERE {where} ORDER BY alert_id LIMIT 1", params, path=path)
...
def query_core(
    ...
    *,
    ...
    # Solr-style core tables resolve the alert from alert/keyword/date only — not participant_id.
    ...
    return _rows(sql + " ORDER BY 1", {"alert_id": ctx["alert_id"]}, path=path)
...
def query_market(
    *,
    ...
    """Market ticks: window + pair from alert context; optional feed source + tick query type."""
    ctx = _context_from_filters(
        alert_id=alert_id,
        participant_id=None,
        keyword=None,
    ...
    return _rows(sql, params, path=path)
...
def search_demo_data(
    *,
    ...
    return {
        "hs_alerts": _rows("SELECT * FROM hs_alerts WHERE alert_id = :alert_id", params, path=path),
        "hs_orders": _rows("SELECT * FROM hs_orders WHERE alert_id = :alert_id ORDER BY book, order_time", params, path=path),
        "hs_exec": _rows("SELECT * FROM hs_exec WHERE alert_id = :alert_id ORDER BY book, exec_time", params, path=path),
        "hs_trades": _rows("SELECT * FROM hs_trades WHERE alert_id = :alert_id ORDER BY book, trade_time", params, path=path),
        "market_ticks": query_market(alert_id=ctx["alert_id"], path=path),
        "comms_messages": _rows(
            """
