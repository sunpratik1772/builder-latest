...
def handle_aitransform(node: dict, ctx: RunContext) -> None:
    ...
    transformation_code = str(cfg.get("transformation_code", ""))
    if not transformation_code:
        if not instructions:
            raise RuntimeError("Missing instructions to generate code")
        transformation_code = _build_fallback_code(instructions)
    ...
    ctx.set(f"{node_id}_generated_code", transformation_code)
