...
def handle_chat(node: dict, ctx: RunContext) -> None:
    ...
    provider = str(cfg.get("provider", cfg.get("channel", "generic"))).lower()
    ...
    if operation == "send":
        ...
        output = {"sendMessage": message, "session_id": session_key}
        # Teams integration path: incoming webhook (simplest) or Graph API.
        if provider in {"teams", "msteams", "microsoft-teams"}:
            webhook_url = str(cfg.get("teams_webhook_url", cfg.get("webhook_url", ""))).strip()
            graph_token = str(cfg.get("access_token", cfg.get("graph_access_token", os.getenv("TEAMS_GRAPH_ACCESS_TOKEN", "")))).strip()
            graph_endpoint = str(cfg.get("graph_endpoint", "")).strip()
            if webhook_url:
                response = requests.post(webhook_url, json={"text": message}, timeout=20)
                response.raise_for_status()
                output.update({"provider": "teams", "deliveryMode": "webhook", "statusCode": response.status_code})
            elif graph_endpoint and graph_token:
                ...
                response.raise_for_status()
                output.update({"provider": "teams", "deliveryMode": "graph", "statusCode": response.status_code})
            else:
                output.update({"provider": "teams", "deliveryMode": "mock", "status": "queued"})
    ...
    if operation == "sendAndWait" and provider in {"teams", "msteams", "microsoft-teams"}:
        wait_events = _to_list(cfg.get("incoming_events", []))
        if wait_events and user_reply is None:
            for event in wait_events:
                if not isinstance(event, dict):
                    continue
                text = str(event.get("text", ""))
                if text:
                    result["chatInput"] = text
                    result["timedOut"] = False
                    break

    merged = [result] if not items else [{**(items[0] if isinstance(items[0], dict) else {}), **result}]
    ctx.set(f"{node_id}_output", merged)
