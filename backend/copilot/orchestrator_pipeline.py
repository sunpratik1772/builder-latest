...
comms_messages (SQLite surveillance fixture — use csv_extract source "comms_messages"):
  ...
  display_post is the chat body text

hs_trades / hs_alerts (SQLite — csv_extract sources "hs_trades", "hs_alerts"):
  trades: trade_id, book, trader_name, notional_usd, risk_flag, currency_pair, …
  alerts: alert_id, trader_name, scenario, description, currency_pair, …
"""
...
agent (per-row scoring — set perRow and cap rows):
  { "prompt": "Score trade anomalies 0-100.", "task": "Return only a number.",
    "perRow": true, "outputColumn": "anomaly_score", "maxRows": 10 }
...
def _repair_linear_edges(nodes: list[dict], edges: list[dict]) -> list[dict]:
    """
    Copilot plans often omit the last edge (e.g. agent → output).
    Chain nodes left-to-right by canvas x when a node has no incoming wire.
    """
    if len(nodes) < 2:
        return edges
...
def finalize_workflow(workflow: dict) -> dict:
    """Public helper: validator-normalized workflow for the Studio canvas."""
...
def run_pipeline_sync(
    ...
    *,
    ...
    for attempt in range(total_attempts):
        ...
        for step in plan.get("thinking_steps") or []:
            if isinstance(step, str) and step.strip():
                emit({"type": "thinking", "step": step.strip()[:200]})
