...
def copilot_chat_stream(req: CopilotChatRequest) -> StreamingResponse:
    """SSE-streamed chat reply.

    The underlying provider call is blocking (`emergentintegrations.LlmChat`
    doesn't expose a streaming interface), so the model response arrives
    in one shot. We then chunk it word-by-word over SSE so the UI can
    render a real typewriter effect rather than a giant late blob.

    Frame shapes mirror `/copilot/generate/stream` for consistency:
      data: {"type": "chunk", "text": "…"}
      data: {"type": "complete", "reply": "<full text>"}
    ...
    def event_source():
        try:
            reply = cp.chat(req.message, session_id=req.session_id) or ""
        except Exception as exc:
        ...
        # Chunk on word boundaries, preserving whitespace so the
        # client can blindly concatenate fragments and get the exact
        # original text back.
        ...
        yield "data: " + json.dumps({"type": "complete", "reply": reply}) + "\n\n"
