...
async def get_current_user(
    ...
    NOTE: We use Cookie/Header directly instead of FastAPI's
    `HTTPAuthorizationCredentials` dependency because the latter can't
...
async def create_session(body: SessionCreateBody, response: Response) -> dict:
    ...
    # Hit Emergent's session-data endpoint to resolve the user. The
    # call is server-to-server so the session_id never round-trips
    ...
    # Upsert user row keyed on email. Custom user_id (UUID) is
    # what every other table references — never Mongo's _id.
    existing = await db.users.find_one({"email": email}, {"_id": 0})
    if existing:
        user_id = existing["user_id"]
        await db.users.update_one(
    ...
    # Persist the session. Use the upstream token verbatim so logging
    # the user out of Emergent invalidates everywhere.
    expires_at = datetime.now(timezone.utc) + _SESSION_TTL
    await db.user_sessions.insert_one(
        {
            "user_id": user_id,
    ...
    # Cookie config: cross-origin in preview deployments, so SameSite=None;
    # Secure mandatory for SameSite=None browsers; httpOnly so JS can't
    # read it. Path=/ so /api requests pick it up.
    response.set_cookie(
        key=_COOKIE_NAME,
        value=upstream_token,
        httponly=True,
        secure=True,
        samesite="none",
        path="/",
