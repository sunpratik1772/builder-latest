...
interface WorkflowStore {
  ...
  /**
   * Append a chunk to the *last* assistant message in the chat. Used
   * by the streaming chat path so the bubble fills in as tokens
   * arrive instead of appearing as one late blob.
   */
...
export const useWorkflowStore = create<WorkflowStore>((set) => ({
  ...
  appendToLastAssistantMessage: (chunk) =>
    set((s) => {
      ...
      // the user message that triggered the stream is right before it,
      // so this is at most O(1) in practice.
