...
/**
 * User avatar — initials disc that opens a tiny popover with the
 * full name/email and a logout button. Anchored top-right of the
 * topbar so it's always visible regardless of which workflow is
 * loaded.
 */
function UserAvatar() {
  ...
  // Top-bar mounts after auth check, but in dev/HMR cases the avatar
  // can mount with `idle` state — kick a refresh so we still render
  // initials instead of "?".
  useEffect(() => {
      ...
      {open && (
        ...
        >
          <div style={{ padding: '10px 10px 8px', borderBottom: '1px solid var(--border-soft)' }}>
            <div className="display truncate" style={{ fontSize: 12.5, fontWeight: 540, color: 'var(--text-0)', letterSpacing: '-0.005em' }}>
              {user?.name ?? 'Not signed in'}
            </div>
            <div className="font-mono truncate" style={{ fontSize: 10.5, color: 'var(--text-3)', marginTop: 2, letterSpacing: 0 }}>
              {user?.email ?? '—'}
            </div>
          </div>
          <button
